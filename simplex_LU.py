...
from utils import pivot_operation, solve, solve_transpose, update
...
def PrimalSimplex(c, A, b, basis=None, nbasis=None, return_basis=False):
    ...
    B, N = A[:, idx_b], A[:, idx_n]
    P, L, U = scipy.linalg.lu(B, permute_l=False, p_indices=False)
    while True:
        ...
        y = solve_transpose(P, L, U, c_b.reshape(m, 1))
        r_n = c_n - N.T @ y
        ...
        a_e = A[:, entering_index].reshape(m, 1)
        d = solve(P, L, U, a_e).ravel()
        if np.all(d < EPS):
        ...
        # Обновляем LU разложение - заменяем l-ый столбец на новый
        L, U = update(P, L, U, a_e, d_pos_args[l])
